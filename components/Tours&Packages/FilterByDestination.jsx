    ...
    <div className="w-full    flex justify-center ">
      <div className=" h-fit    bg-gray-100 w-[80%] sm:w-full rounded-3xl">
        <div className="flex justify-between items-center p-5">
          <h1 className="text-[#165064] text-[20px] ">Destination</h1>
          <h1 className={` transition-all  duration-500 ${isOpened && "rotate-180"}`} onClick={() => setIsOpened(!isOpened)}>
            ...
            {Destination.map((item, i) => {
              return(
                <div key={i} className="mb-3 flex items-center gap-3">
                  <div>
                    <input className="w-[20px] h-[20px]  translate-y-[2px]" type="checkbox" value={false}/>
                  </div>
