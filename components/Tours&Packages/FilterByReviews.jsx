...
const Filter = () => {
  ...
  const StarComp = ({ number }) => {
    return (
      <div className="flex items-center gap-3">
        <input
          className="w-[20px] h-[20px]  translate-y-[1px]"
          type="checkbox"
          value={false}
        />
        <div className="flex gap-2">
          {Array(number)
            ...
            .map((_, i) => {
              return (
                <div key={i}>
                  <FaStar className="text-[orange] text-[15px] " />
                </div>
              );
            })}
          {Array(5 - number)
            ...
            .map((_, i) => {
              return (
                <div key={i}>
                  <FaStar className="text-gray-300 text-[15px]" />
                </div>
              );
            })}
        </div>
    ...
    <div className="w-full    flex justify-center ">
      <div className=" h-fit    bg-gray-100 w-[80%] sm:w-full rounded-3xl">
        <div className="flex justify-between items-center p-5">
          <h1 className="text-[#165064] text-[20px] ">Review Score</h1>
          <h1
